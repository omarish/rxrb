...
RSpec.describe Rxrb::Rx do
  it "passes all the old tests" do
    test_data   = { }
    test_schema = { }

    Dir.open('./spec/fixtures/data').each { |file|
      next if file =~ /\A\./

      json = File.open("./spec/fixtures/data/#{file}").read
    ...
    Find.find('./spec/fixtures/schemata') { |path|
      next unless File.file?(path)

      leaf = Pathname.new(path).
            relative_path_from( Pathname.new('./spec/fixtures/schemata') ).to_s
        ...
        schema_test_desc[pf].each_pair { |source, entries|
          ...
          entries.each_pair { |entry, want|
            result = begin
              ...
              true
            rescue Rxrb::Rx::ValidationError => e
              false
            end
            ok = (pf == 'pass' and result) || (pf == 'fail' and !result)
